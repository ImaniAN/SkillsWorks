<fieldset>
	<legend for="form-extra-opts" class="sr-only"
		>idk man, you have a lot of things to figure out to do</legend
	>
	<!--START:()-->
	<div class="text-base font-medium text-slate-900" aria-hidden="true">
		idk man, you have a lot of things to figure out to do
	</div>
	<!--END:()-->

	<!--START:()-->
	<div class="mt-4 space-y-4">
		<div class="flex items-start">
			<div class="flex items-center h-5">
				<input
					type="checkbox"
					checked
					name="form-extra-opts-sa-id"
					id="form-extra-opts-sa-id"
					class="bg-red-200 border-dashed border-2 rounded border-teal-400 text-teal-400 ml-1 w-5 h-5 ease-linear transition-all duration-150"
				/>
			</div>
			<!--END:()-->

			<!--START:()-->
			<div class="ml-3 text-sm">
				<label for="form-extra-opts-sa-id" class="font-medium text-slate-700"
					>Do you have a South African ID?</label
				>
				<p class="text-slate-500">
					This is Lorem ipsum place holder Text!!! dolor sit ame, consectetur
					adipisicing sit ame, consectetur adipisicing?
				</p>
			</div>
		</div>
		<!--END:()-->

		<!--START:()-->
		<div class="flex items-start">
			<div class="flex items-center h-5">
				<input
					type="checkbox"
					checked
					name="form-extra-opts-disabilities"
					id="form-extra-opts-disabilities"
					class="bg-red-200 border-dashed border-2 rounded border-teal-400 text-teal-400 ml-1 w-5 h-5 ease-linear transition-all duration-150"
				/>
			</div>
			<!--END:()-->

			<!--START:()-->
			<div class="ml-3 text-sm">
				<label
					for="form-extra-opts-disabilities"
					class="font-medium text-slate-700">Are you disabled?</label
				>
				<p class="text-slate-500">
					This is Lorem ipsum place holder Text!!! dolor sit ame, consectetur
					adipisicing?
				</p>
			</div>
		</div>
		<!--END:()-->

		<!--START:()-->
		<div class="flex items-start">
			<div class="flex items-center h-5">
				<input
					type="checkbox"
					checked
					name="form-extra-opts-cc"
					id="form-extra-opts-cc"
					class="bg-red-200 border-dashed border-2 rounded border-teal-400 text-teal-400 ml-1 w-5 h-5 ease-linear transition-all duration-150"
				/>
			</div>
			<!--END:()-->

			<!--START:()-->
			<div class="ml-3 text-sm">
				<label for="form-extra-opts-cc" class="font-medium text-slate-700"
					>Can you provide certified copies on request?</label
				>
				<p class="text-slate-500">
					This is Lorem ipsum place holder Text!!! dolor sit ame, consectetur
					adipisicing sit ame, consectetur adipisicing?
				</p>
			</div>
		</div>
		<!--END:()-->

		<!--START:()-->
		<div class="flex items-start">
			<div class="flex items-center h-5">
				<input
					type="checkbox"
					checked
					name="form-extra-drivers-licence"
					id="form-extra-drivers-licence"
					class="bg-red-200 border-dashed border-2 rounded border-teal-400 text-teal-400 ml-1 w-5 h-5 ease-linear transition-all duration-150"
				/>
			</div>
			<!--END:()-->

			<!--START:()-->
			<div class="ml-3 text-sm">
				<label
					for="form-extra-drivers-licence"
					class="font-medium text-slate-700">Do you have drivers licence?</label
				>
				<p class="text-slate-500">
					This is Lorem ipsum place holder Text!!! dolor sit ame, consectetur
					adipisicing?
				</p>
			</div>
		</div>
		<!--END:()-->

		<!--START:()-->
		<div class="flex items-start">
			<div class="flex items-center h-5">
				<input
					type="checkbox"
					checked
					name="form-extra-criminal-record"
					id="form-extra-criminal-record"
					class="bg-red-200 border-dashed border-2 rounded border-teal-400 text-teal-400 ml-1 w-5 h-5 ease-linear transition-all duration-150"
				/>
			</div>
			<!--END:()-->

			<!--START:()-->
			<div class="ml-3 text-sm">
				<label for="offers" class="font-medium text-slate-700"
					>Do you have a criminal record?</label
				>
				<p class="text-slate-500">
					This is Lorem ipsum place holder Text!!! dolor sit ame, consectetur
					adipisicing sit ame, consectetur adipisicing?
				</p>
			</div>
			<!--END:()-->
		</div>
		<!--END:()-->
	</div>
	<!--END:()-->
</fieldset>
